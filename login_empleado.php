...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['form_type']) && $_POST['form_type'] == 'login') {
    ...
    if (empty($email) || empty($password_ingresada)) {
        ...
        if ($conexion) {
            ...
            $sql = $conexion->prepare("SELECT empleado_id, nombre, password, rol, activo FROM empleados WHERE email = ?");
            ...
            if ($sql === false) {
                ...
                if ($resultado->num_rows === 1) {
                    ...
                    if (!$empleado['activo']) {
                        $error_login = "Cuenta desactivada. Contacte al administrador.";
                    }
                    // Verificar contraseña
                    elseif (password_verify($password_ingresada, $empleado['password'])) {
                        // Login correcto
...
// PROCESAR REGISTRO DE EMPLEADOS 
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['form_type']) && $_POST['form_type'] == 'register_empleado') {
    ...
    if (empty($nombre) || empty($primer_apellido) || empty($segundo_apellido) || 
        empty($email) || empty($telefono) || empty($password) || empty($password2) || empty($puesto)) {
    ...
    elseif (!preg_match('/^\d{10}$/', $telefono)) {
        ...
        if ($conexion) {
            $check = $conexion->prepare("SELECT empleado_id FROM empleados WHERE email = ?");
            ...
            if ($resultado->num_rows > 0) {
                ...
                // 11. Insertar empleado con valores por defecto para los otros campos
                $sql = $conexion->prepare("
                    INSERT INTO empleados 
                    (nombre, primer_apellido, segundo_apellido, email, password, 
                     telefono, puesto, rol, es_responsable, fecha_contratacion, 
                     salario, activo, fecha_creacion)
                    VALUES (?, ?, ?, ?, ?, ?, ?, 'EMPLEADO GENERAL', FALSE, CURDATE(), 0.00, TRUE, NOW())
                ");
                ...
                if ($sql === false) {
                    ...
                    if ($sql->execute()) {
                        ...
                        error_log("Error al registrar empleado: " . $sql->error);
...
<head>
  ...
  <title>Login Empleado - SAFI Electrónicos</title>
...
<body>
  <div class="empleado-badge">EMPLEADO</div>
  ...
  <div class="container">
    ...
    <div class="container-form">
      <form class="sign-in" method="POST">
        ...
        <span>Acceso para empleados del sistema</span>
    ...
    <!-- Formulario de Registro de Empleados - TODOS LOS CAMPOS OBLIGATORIOS -->
    <div class="container-form">
      <form class="sign-up" method="POST">
        <input type="hidden" name="form_type" value="register_empleado">
        ...
        <div class="container-input">
          ...
          <select name="puesto" required>
            ...
            <option value="Vendedor" <?php echo (isset($_POST['puesto']) && $_POST['puesto'] == 'Vendedor') ? 'selected' : ''; ?>>Vendedor</option>
            <option value="Almacenista" <?php echo (isset($_POST['puesto']) && $_POST['puesto'] == 'Almacenista') ? 'selected' : ''; ?>>Almacenista</option>
            <option value="Administrativo" <?php echo (isset($_POST['puesto']) && $_POST['puesto'] == 'Administrativo') ? 'selected' : ''; ?>>Administrativo</option>
            <option value="Contador" <?php echo (isset($_POST['puesto']) && $_POST['puesto'] == 'Contador') ? 'selected' : ''; ?>>Contador</option>
          </select>
        </div>
    ...
    <div class="container-welcome">
      <div class="welcome-sign-up welcome">
        ...
        <p>Registre nuevos empleados en el sistema con todos sus datos.</p>
        <button class="button" id="btn-sign-up" type="button">REGISTRAR</button>
      </div>

      <div class="welcome-sign-in welcome">
        <h3>¡Bienvenido Empleado!</h3>
        <p>Ingrese con sus credenciales para acceder al sistema de empleados.</p>
  ...
  if (formRegister) {
    formRegister.addEventListener("submit", function (e) {
      ...
      const requiredInputs = formRegister.querySelectorAll('input[required], select[required]');
      let allFilled = true;
      ...
      // Validar teléfono (10 dígitos exactos) - AHORA ES OBLIGATORIO
      ...
      const puestoSelect = document.querySelector("select[name='puesto']");
